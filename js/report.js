document.addEventListener("DOMContentLoaded", async () => {
  ...
  if (toneDisplay) toneDisplay.textContent = tone;

  const chart = JSON.parse(localStorage.getItem("chart") || "null");
  const narrative = JSON.parse(localStorage.getItem("narrative") || "null");
  const reportBody = document.getElementById("report-body");

  if (!chart || !narrative || !reportBody) {
    if (reportBody) {
      reportBody.innerHTML = "<p>⚠️ 尚未生成命盤，請先回到首頁輸入資料。</p>";
    }
    return;
  }
  ...
  try {
    ...
    if (res.ok) {
      reportBody.innerHTML = await res.text();
      return;
    }

    console.warn("Report endpoint responded with", res.status, res.statusText);
  } catch (error) {
    console.warn("Falling back to client-side rendering:", error);
  }

  reportBody.innerHTML = renderReport(tone, chart, narrative);
});
...
function renderReport(tone, chart, narrative) {
  switch (tone) {
    case "healing":
      return generateHealingReport(chart, narrative);
    case "military":
      return generateMilitaryReport(chart, narrative);
    case "mythic":
      return generateMythicReport(chart, narrative);
    case "savage":
      return generateSavageReport(chart, narrative);
    default:
      return "<p>⚠️ 無法辨識語氣，請重新選擇。</p>";
  }
}
...
function generateHealingReport(chart, narrative) {
  return `
    ...
    <div class="report-section">
      <h2>🧭 四柱能量</h2>
      ${Object.entries(chart.pillars)
        .map(
          ([key, val]) => `
        <p><strong>${key}柱：</strong> ${val.pillar}（${val.gan}${val.zhi}，${
          chart.tenGods.find((g) => g.includes(val.gan)) || "無十神"
        }）</p>
      `,
        )
        .join("")}
    ...
    <div class="report-section">
      <h2>🌈 五行分佈</h2>
      <p>${Object.entries(chart.fiveElements)
        .map(([el, val]) => `${el}：${val}`)
        .join("　")}</p>
    ...
    <div class="report-section">
      ...
      ${Object.entries(chart.pillars)
        .map(
          ([key, val]) => `
        <p>${key}柱：${val.pillar} → ${
          narrative[key]?.commander || "未知主將"
        }（${chart.tenGods.find((g) => g.includes(val.gan)) || "無十神"}）</p>
      `,
        )
        .join("")}
    </div>
        ...
        .map(([pillar, data]) => {
          ...
          const story =
            typeof data?.story === "string"
              ? data.story.replace(/你/g, "你這傢伙")
              : "";
          return `
        <div class="report-card">
          <h3>${pillar}柱 · ${commander}</h3>
          <p>${story}</p>
        </div>
      `;
        })
...
(function renderEnergyBars() {
  try {
    ...
    function createRow(label, value, max) {
      ...
      el.innerHTML = `
        ...
        <div class="energy-track">
          <div class="energy-fill" style="width: ${pct * 100}%;"></div>
        </div>
    ...
    ["木", "火", "土", "金", "水"].forEach((key) => {
      container.appendChild(createRow(key, elements[key] || 0, maxElem));
    });

    const badges = document.createElement("div");
    badges.className = "energy-badges";
    badges.innerHTML = `
      <span class="energy-badge">陰：${yinYang["陰"] || 0}</span>
      <span class="energy-badge">陽：${yinYang["陽"] || 0}</span>
    `;
    container.appendChild(badges);
  } catch (error) {
    console.warn("Energy bars skipped:", error);
  }
})();
  ...
  try {
    if (!localStorage.getItem("reportData")) {
      fetch("data/sample.json")
        ...
        .catch((error) => {
          console.warn("Unable to preload sample report data:", error);
        });
    }
  } catch (error) {
    console.warn("Sample data check failed:", error);
  }
})();
